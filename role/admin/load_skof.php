<?
  ...
  if($action=="import"){
    ...
    if (!file_exists($tmpFile=$_FILES["fileImp"]['tmp_name'])) {
      ...
      if ($db) {
          ...
          $querySelect = "SELECT * FROM `skof` WHERE `kod`= ?";
          $queryUpdate = "UPDATE `skof` SET `nu`=? WHERE `kod`=?";
          $queryInsert = "INSERT INTO `skof`(`kod`, `nu`) VALUES (?,?)";
          ...
          if((!mysqli_stmt_prepare($stmtSelect, $querySelect))||(!mysqli_stmt_prepare($stmtInsert, $queryInsert))||(!mysqli_stmt_prepare($stmtUpdate, $queryUpdate)))
          {
            ...
            mysqli_stmt_bind_param($stmtUpdate, "ss",$nuU,$kdUs);
  ...
  $qeruStrPaginathion="SELECT COUNT(*) as resC FROM `skof` ".$whereStrPa;
  ...
  $qeruStr="SELECT * FROM `skof` ".$whereStr;
  //echo $qeruStr;
  ...
  require_once('template/load_skof.php');
?>
