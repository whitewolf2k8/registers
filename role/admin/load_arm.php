...
if($action=="importContact"){
    ...
    if (!file_exists($tmpFile=$_FILES["fileImpContact"]['tmp_name'])) {
		...
		$arrType=array("0"=>"OT","1"=>"DT","2"=>"PF","3"=>"EMAIL");	
		foreach($arrType as $key => $value){
			$db = dbase_open($tmpFile, 0);
			if ($db) {
				$countInsert=0;
				// чтение некотрых данных	
				$querySelectOrg = "SELECT id FROM `organizations` WHERE `kd`=? and `kdmo`=?";
				...
				$rowCount=dbase_numrecords ($db);
				if((!mysqli_stmt_prepare($stmtSelect, $querySelectOrg))||(!mysqli_stmt_prepare($stmtInsert, $queryInsert))||(!mysqli_stmt_prepare($stmtSelectContact, $querySelectContact)))
				{
					echo " Помилка Пыдготовки запиту \n <br>";
				} else {
					mysqli_stmt_bind_param($stmtInsert, "isi",$idOrg,$data,$type);
					mysqli_stmt_bind_param($stmtSelect, "ii", $kdS, $kdmoS);
					mysqli_stmt_bind_param($stmtSelectContact, "isi",$idOrgS,$dataS,$typeS);
					...
					for($i=1;$i<=$rowCount;$i++){
						...
						if ($key<3){
							...
							if($row[$value]!=0) {
								...
								if(mysqli_num_rows($result)>0)
								{
									$org = mysqli_fetch_array($result, MYSQLI_ASSOC);
									$idOrgS=$org["id"];
									$dataS=$row[$value];
									$typeS=(($key<2)?0:1);	
									mysqli_stmt_execute($stmtSelectContact);
									$resultStr = mysqli_stmt_get_result($stmtSelectContact);
								...
								$ERROR_MSG.=" Пдприэмство не знайдено kd ".$row["KD"]." kdmo ".$row["KDMO"]." . <br>";
								continue;
							}
							}
						}else{
