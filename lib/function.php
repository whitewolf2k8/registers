<?
  ...
  function connectingDb()
  {
    ...
    if (!$link) {
        die('Îøèáêà ïîäêëþ÷åíèÿ (' . mysqli_connect_errno() . ') ');
    }
    return $link;
  }
  ...
  function checkUser($link, $username, $password)
  {
    ...
    {
      echo "Îøèáêà ïîäãîòîâêè çàïðîñà\n";
    }
    else
    {
  ...
  function changeCodingPage($string)
  {
	...
	function isCyrilic($text) {
	    return preg_match('/[À-ßà-ÿ¨¸¯¿¥´0-9]/', $text);
	}
  ...
  function calcTimeRun($startTime,$endTime){
    ...
    return "<br> Âèêîíàííÿ ñêðèïòà çàéíÿëî  $hours ãîäèí $minutes õâèëèí $seconds ñåêóíä";
  }
	...
	function errorMesKodKved10(){
		return "Ââåäåíèé êîä ÊÂÅÄ íå â³äïîâ³äàº øàþëîíó ÷÷.÷÷. Íåìîæëèâî âèêîíàòè ïîøóê.";
	}
	...
	function formatKdKise14($string){
    	...
    	case 4:
					...
					return "Ââåäåíå ÷èëî íå â³äïîâ³äàº ôîðìàòó ÷÷÷÷";
        	break;
			case 1:
					if($string==0){
						return $string;
	...
	function formatKodKise14($string){
		$string=str_replace(" ","",$string);
		$result="";
		$errorM= "Ââåäåíå ÷èëî íå â³äïîâ³äàº ôîðìàòó S.÷÷÷÷÷";
		switch (iconv_strlen($string, "Windows-1251")) {
			case 3:
	...
	function getPaginator($total,$count=10,$now){
		...
		//ïðîâåðêà íà íóëåâîå êîëè÷åñòâî ñòðàíèö â âûáîðå ïðè ïåðâîé çàãðóçêå ïàãèíàòîðà
		...
		$result.="<div class='pagination'><p> Â òàáëèöþ âèáðàíî ".
		$total." çàïèñ³â. Ïîêàçàòè â òàáëèö³ ïî "
			.'<select name="limits" size="1" onchange="submitFormLim(this.value)">';
		foreach($arrayCount as $v) {
			$result .= '<option value="'.$v.'"'.($count==$v ? ' selected="selected"' : '').'>'.($v==0 ? 'Âñå' : $v).'</option>';
		}
 		$result.= '</select></p>';
